...
const TwitchApi = class TwitchApi {
  ...
  async setupAxios () {
    let finalToken
    const dbToken = await ApiToken.findOne()
    if (!dbToken) {
      finalToken = await this.getOauthToken()
    } else {
      const valid = await this.validateToken(dbToken.token)
      finalToken = valid ? dbToken.token : await this.getOauthToken()
    }
  ...
  async getOauthToken () {
    try {
      ...
      const token = await ApiToken.create({ token: data.access_token })
      return token.token
    } catch (error) {
      return { error }
    }
